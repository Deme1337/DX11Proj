...
public:
	...
	explicit TinyTextContext_c();

	// Initialises the context
	bool Init(ID3D11Device * device, ID3D11DeviceContext * inDevcon, size_t characterCapacity);
	...
	bool Print(int x, int y, const char * text, DWORD colour = DefaultColour);
	bool Print(size_t maxCharacterCount, int x, int y, const char * text, DWORD colour = DefaultColour);
	...
	TInitZero<int> CursorX;
	TInitZero<int> CursorY;
	// 0 if there is no key or it was comsumed
	TInitZero<int> PendingKey;
	// bit 1: shift
	TInitZero<int> PendingKeyState;

	// @return 0 if there isno key pending
	uint8 GetPendingASCIIKey();

private:

	TInitZero<uint32> Width;
	TInitZero<uint32> Height;
