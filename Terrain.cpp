...
bool CTerrain::Initialize(ID3D11Device* device, char* setupFilename)
{
	bool result;
...
bool CTerrain::Render(ID3D11DeviceContext* deviceContext)
{
...
bool CTerrain::LoadSetupFile(char* filename)
{
	int stringLength;
	ifstream fin;
	char input;
...
bool CTerrain::SetTerrainTextures(CDeviceClass* devclass, char* path1, char* path2)
{
	m_TerrainTexture[0] = new CTextureTA();
	m_TerrainTexture[1] = new CTextureTA();
	this->m_TerrainTexture[0]->LoadFreeImage(devclass->GetDevice(), devclass->GetDevCon(), path1);
	this->m_TerrainTexture[1]->LoadFreeImage(devclass->GetDevice(), devclass->GetDevCon(), path2);
...
bool CTerrain::LoadRawHeightMap()
{
	int error, i, j, index;
	FILE* filePtr;
...
bool CTerrain::BuildTerrainModel()
{
	...
	for (j = 0; j<(m_terrainHeight - 1); j++)
	{
		for (i = 0; i<(m_terrainWidth - 1); i++)
		{
			...
			index++;
			int counst = m_terrainWidth + m_terrainHeight;
			int randomN = rand() % counst + 1;
			if (randomN % 25 == 0)
			{
				this->terrainmodels.push_back(XMVectorSet((float)m_heightMap[index1].x, (float)m_heightMap[index2].y, (float)m_heightMap[index3].z, 1.0f));
			}
		}
	}
...
void CTerrain::ShutdownTerrainModel()
{
...
void CTerrain::RenderBuffers(ID3D11DeviceContext* deviceContext)
{
	unsigned int stride;
	unsigned int offset;
